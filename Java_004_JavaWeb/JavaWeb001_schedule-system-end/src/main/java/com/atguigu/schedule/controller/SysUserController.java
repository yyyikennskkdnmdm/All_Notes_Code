...
public class SysUserController  extends BaseContoller {
    ...
    protected void login(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if(null == loginUser){// 用户名不存在
            result =Result.build(null,ResultCodeEnum.USERNAEM_ERROR);

        } else if(! MD5Util.encrypt(sysUser.getUserPwd()).equals(loginUser.getUserPwd())){// 密码错误
            result =Result.build(null,ResultCodeEnum.PASSWORD_ERROR);
        }else{
            // TODO 登录程序,将用户uid和username响应给客户端
